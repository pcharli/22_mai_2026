...
handleOffline(); //check if the user is offline
...
//Displays the events in the result div or an error message if there is no events
async function displayEvents(events) {
  ...
  // Si les inputs sont vides (cas offline), on utilise la date en localStorage
  if (!day || !month) {
  ...
  // Mettre à jour le titre avec la date (si année est envoyée par le formulaire)
...
//Récupération des events sur Wikipedia
async function fetchEvents() {
  ...
  if ($yearInput.value !== "" && events != undefined) {
    //on la converti en nombre (sinon c'est du texte)
    const year = Number.parseInt($yearInput.value);
    //on filtre les events par année
...
//Création d'une carte pour chaque event
async function createEventcard(eventData) {
  const pages = eventData.pages; //tableau des pages
  const image = pages[0].thumbnail?.source; //image de la page 0
  const imageAlt = pages[0].title; //titre de la page 0
  const articleLink = pages[0].content_urls.desktop.page; //lien de la page 0

  pages.splice(0, 1); //on retire le premier element du tableau

  //création des liens "Voir aussi"
  let links = pages.map(
    (page) =>
  ...
  if (links.length == 0) {
    ...
    //transforme un tableau de liens en une ligne avec comme séparateur ','
    links = links.reduce((a, value) => (a = a + ", " + value));
  }
  // creation de la card via <details>
  $resultContainer.innerHTML += `
    <details class="card">
      <summary class="card__title">
        ${eventData.year} - ${imageAlt.replace(/_/g, ' ') /*remplace les _ par des espaces*/}
      </summary>
      <div>
...
// Vérifie si l'utilisateur est en ligne et si l'API répond
...
//Gère la connexion hors ligne : charge les dernières données récupérées dans le localStorage
//et masque le formulaire en le remplaçant par un message d'erreur
//Ajoute un écouteur pour rafraîchir la page quand la connexion revient
